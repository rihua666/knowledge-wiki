...
> 来源: https://mp.weixin.qq.com/s/H-hggbJAMBTuambCzNE4OQ
> 抓取日期: 2026-04-13
> 标签: #cli #agent #设计原则 #工具设计 #钉钉 #飞书
...
原则六：退出码控制
退出码对 Agent 来说是控制流本身。只用 0 和 1 不够：0=成功、1=一般错误、2=参数错误、3=资源不存在、4=权限不足、5=冲突/已存在。退出码必须跨版本稳定。
...
飞书 CLI 评分：noun-verb 结构清晰，schema 命令完善，五种输出格式。可改进：非 TTY 未自动切 JSON，退出码缺乏文档化。
钉钉 CLI 评分：--yes 语义自描述，批量熔断防 Agent 失控。可改进：缺少 schema 自省，帮助文本缺少示例。

开源项目：https://github.com/Johnixr/agent-cli-guide
